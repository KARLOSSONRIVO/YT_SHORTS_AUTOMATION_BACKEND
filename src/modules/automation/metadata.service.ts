...
export class PlatformMetadataService {
  public build(candidate: TopicCandidate, profile: NicheProfile, platform: string) {
    ...
    const isSerializedStory = profile.id === "original_serialized_mystery";
    ...
    const seriesLine = isPhilippineHistory ? "Hidden Philippine History" : isSerializedStory ? "Original Serialized Mystery Stories" : undefined;
    const provenanceLine = isSerializedStory ? "Original fictional story generated for this series." : "Sources are recorded in the content history.";
    const description = [seriesLine, candidate.summary, provenanceLine, hashtags.join(" ")]
